...
## Функциональность
  ...
  - API для подключения любого привода оси Z (уже реализованы 2 типа драйверов для шагового двигателя: биполярное 4-х проводное включение и 2-х проводный "умный" драйвер (STEP + DIRECTION));
  ...
  - Детектирование касания металла (не требуется предварительно выставлять начальную высоту, достаточно просто указать ее в настройках);
...
Отсюда следует, что перед работой следует кроме всего прочего настроить в ЧПУ некоторую задержку между зажиганием плазмы и началом реза (перемещение по осям XY). Большинство станков ЧПУ имеют такую настройку.
...
В скобках указаны выводы ATmega328P и Arduino.
  ...
  - **FEEDBACK** (PC1, A1) - аналоговый сигнал напряжения дуги, приведенный к интервалу входных напряжений АЦП ATmega328P (0-5 Вольт, желательно с запасом), например, резистивным делителем. Резкие скачки и перепады, а также шумы в сигнале крайне жеалтельно устранить установкой фильтра;
